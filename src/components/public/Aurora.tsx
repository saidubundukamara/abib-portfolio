...
export default function Aurora({
  ...
  useEffect(() => {
    ...
    const colorStopsArray = (colorStops ?? ['#F46C38', '#0099FF', '#C5FF41']).map((hex) => {
    ...
    const update = (t: number) => {
      ...
      const { speed: spd = 1.0, amplitude: amp = 1.0, blend: blnd = 0.5, colorStops: stops } =
        propsRef.current
      if (program) {
        program.uniforms.uTime.value = t * 0.001 * spd
        program.uniforms.uAmplitude.value = amp
        program.uniforms.uBlend.value = blnd ?? blend
        if (stops) {
